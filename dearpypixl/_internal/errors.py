...
if TYPE_CHECKING:
    from dearpypixl.itemtypes import ItemT, WidgetItemT
...
def err_item_not_created(item: ItemT, arguments: dict):
    ...
    _signature = inspect.signature(itemtype.__dearpypixl__.command)
    ...
    if "parent" in signature.parameters:
        ...
        # is suitable for this item.
            ...
            parent_type  = registry.get_itemtype(dpg.get_item_info(parent)["type"])
...
def err_if_root_item(item: ItemT) -> None | TypeError:
    ...
    """
    if item.__dearpypixl__.is_root_item:
