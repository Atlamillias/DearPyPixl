...
from dearpypixl.application import Application
from dearpypixl.containers import Window, Group, ChildWindow
from dearpypixl.widgets import Text, InputText
...
class PyConsole(InteractiveInterpreter):
    ...
    def __init__(
        ...
        with self.parent:
            ...
            with ChildWindow(autosize_x=True) as self.stdout:
                Text(self.banner_msg)
            dearpygui.add_dummy(height=1)
            with ChildWindow(no_scrollbar=True) as self.stdin_parent:
